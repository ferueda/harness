...
Live implementation runs one implementer and writes candidate changes plus an
internal review ref. `harness factory implementation review` owns the durable
three-role review/remediation loop, cumulative candidate refs, resumable
failure recovery, and PR-ready handoff. Linear `--apply` moves an eligible first
run or retry to `Implementing`, then posts the terminal handoff or failure
projection; without `--apply` it does not mutate Linear or create a
