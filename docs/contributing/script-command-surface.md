...
| Source CLI                  | `bin/harness.ts`             | `harness init`, `harness factory status`, `harness factory linear list`, `harness factory linear fetch`, `harness factory linear create`, `harness factory triage`, `harness factory planning run`, `harness factory planning publish`, `harness factory planning mark-plan-merged`, `harness factory implementation run`, `harness factory implementation review`, `harness run change-review`, `harness run factory-triage`, `harness run plan-review`, `harness runs prune`, `harness models`, `harness skills install` | User-facing harness workflow commands, tracker read/create adapters, local factory inbox management, implementation and review stations, optional Linear `--apply`, and planning handoff metadata commands. Workflow implementations live in `workflows/*.workflow.ts`. |
...
| Checking with ignored artifacts         | `make build`, `pnpm build`, `make smoke-dist`, `pnpm smoke:dist`, `make check`, `make check-v`, `make check-ci`, `pnpm check`, `pnpm check:v`, `pnpm check:ci`, `harness factory triage --dry-run`, `harness factory planning run --dry-run`, `harness factory implementation run --dry-run` | Build and gate paths may refresh ignored `dist/`. Smoke and full check paths also run dry-run standalone reviews under workspace-local `.harness/runs/reviews/<run-id>/`. Eligible factory dry-runs write durable run artifacts and retained workspace-local planner scratch, do not invoke providers, and do not mutate lifecycle state. Planning dry-run preserves the `run-started` stderr progress line while suppressing workflow events and lifecycle writes. Triage with prior `triage.completed` exits before run creation unless `--rerun` is supplied. Linear-backed dry-runs still perform the required live Linear read without mutating Linear.                                                                                                                                                                                                                                                         |
...
| Factory artifact writing                | `harness run factory-triage`, `harness factory triage`, `harness factory planning run`, `harness factory planning publish`, `harness factory planning mark-plan-merged`, `harness factory implementation run`, `harness factory implementation review`                                       | `harness factory ...` station commands write lifecycle state and run evidence under `${XDG_DATA_HOME:-~/.local/share}/harness/store/projects/<repo-id>/` by default. Factory planning keeps mutable planner scratch under ignored workspace `.harness/factory-drafts/<run-id>/`, then Harness alone publishes canonical and immutable evidence to the durable store. Live lifecycle writes are serialized per work item; dry-run station input inspects only and never rebuilds state. Implementation `--apply` additionally requires `LINEAR_API_KEY` and projects only its guarded `Implementing`/terminal marker updates. Factory planning and implementation review nest review evidence under that store's `runs/reviews/`; implementation review does not mutate Linear. Standalone review workflows stay workspace-local, and low-level `harness run factory-triage` retains its workspace-local run default. |
...
`planning mark-plan-merged`, `implementation run`, and `implementation review` write factory lifecycle
...
- `harness factory implementation review --help`
