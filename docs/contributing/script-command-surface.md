...
| Checking with ignored artifacts         | `make build`, `pnpm build`, `make smoke-dist`, `pnpm smoke:dist`, `make check`, `make check-v`, `make check-ci`, `pnpm check`, `pnpm check:v`, `pnpm check:ci`, `harness factory triage --dry-run`, `harness factory planning run --dry-run`, `harness factory implementation run --dry-run` | Build and gate paths may refresh ignored `dist/`. Smoke and full check paths also run dry-run standalone reviews under workspace-local `.harness/runs/reviews/<run-id>/`. Factory dry-runs write run artifacts under the durable factory store, do not invoke providers, and do not mutate lifecycle state. Linear-backed dry-runs still perform the required live Linear read without mutating Linear.                                                                                                      |
