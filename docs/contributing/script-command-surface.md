...
| Factory artifact writing                | `harness factory triage`, `harness factory planning run`, `harness factory planning publish`, `harness factory planning mark-plan-merged`, `harness factory implementation run`                                                                                                 | Writes action lifecycle state and recovery evidence under `${XDG_DATA_HOME:-~/.local/share}/harness/store/projects/<repo-id>/` by default. Phase runs execute at most one handler, persist its terminal result, print the next reaction, and exit. Implementation also creates an immutable candidate ref and promotes only a passing reviewed commit.                                                                                              |
