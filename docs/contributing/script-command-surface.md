...
| Source CLI                  | `bin/harness.ts`             | `harness init`, `harness factory status`, `harness factory dispatch`, `harness run change-review`, `harness run factory-triage`, `harness run plan-review`, `harness runs prune`, `harness models`, `harness skills install` | User-facing harness workflow commands and local factory inbox management. Workflow implementations live in `workflows/*.workflow.ts`.                          |
...
| Checking with ignored artifacts | `make build`, `pnpm build`, `make smoke-dist`, `pnpm smoke:dist`, `make check`, `make check-v`, `make check-ci`, `pnpm check`, `pnpm check:v`, `pnpm check:ci`, `harness factory dispatch --dry-run`             | Build and gate paths may refresh ignored `dist/`. Smoke and full check paths also run dry-run `change-review` against the harness checkout and leave ignored `.harness/runs/reviews/<run-id>/` artifacts. Factory dispatch dry-run writes ignored `.harness/runs/factory/<run-id>/` artifacts. |
...
| Factory artifact writing        | `harness run factory-triage`                                                                                                                                                                                     | Live and dry-run factory triage commands write ignored run artifacts under `.harness/runs/factory/<run-id>/`. Dry-run writes placeholders and does not invoke a provider.                                                                                                                      |
| Factory inbox writing           | `harness factory dispatch`                                                                                                                                                                                       | Live dispatch writes factory run artifacts and moves inbox files to `processed/` or `failed/`. `--dry-run` writes run artifacts but leaves inbox files unmoved.                                                                                                                                |
