...
| Factory artifact writing                | `harness factory triage`, `harness factory planning run`, `harness factory planning publish`, `harness factory planning mark-plan-merged`                                                                                                                                       | Writes action lifecycle state and recovery evidence under `${XDG_DATA_HOME:-~/.local/share}/harness/store/projects/<repo-id>/` by default. Triage and planning run at most one handler, persist its terminal result, print the next reaction, and exit. Planning publication commands append explicit lifecycle boundaries.                                                                                                                         |
