...
|---------|------------|-----------------|----------|
| Make targets | `Makefile` | `make ...` | Local development gates and wrappers around package scripts. |
| pnpm scripts | `package.json` | `pnpm ...` | Direct package-level commands under Make targets. |
...
- Treat `bin/harness.ts`, `scripts/smoke-dist.ts`, `workflows/*.ts`,
  `skills/*/scripts/*`, and `skills/*/scripts/install.sh` as executable command
  surfaces.
...
- Plan C owns drift tests and enforcement for command names, script inventory,
  private-path leakage, and docs/gate coverage.
