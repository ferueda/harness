...
| Source CLI                  | `bin/harness.ts`             | `harness init`, `harness factory status`, `harness factory linear list`, `harness factory linear fetch`, `harness factory linear create`, `harness factory triage`, `harness run change-review`, `harness run factory-triage`, `harness run plan-review`, `harness runs prune`, `harness models`, `harness skills install` | User-facing harness workflow commands, tracker read/create adapters, local factory inbox management, and one manually stepped triage action. Planning, publication, and implementation commands are unavailable. Workflow implementations live in `workflows/*.workflow.ts`. |
...
| Factory artifact writing                | `harness run factory-triage`, `harness factory triage`                                                                                                                                                                                                                          | `harness factory triage` writes action lifecycle state and recovery evidence under `${XDG_DATA_HOME:-~/.local/share}/harness/store/projects/<repo-id>/` by default. Live writes are serialized per work item; dry-run inspects only. The command runs at most one handler, blocks until it ends, persists its terminal result, prints the reaction, and exits. PR 1 has no executable downstream Factory command, and route evidence may omit one. Low-level `harness run factory-triage` retains its workspace-local run default. |
