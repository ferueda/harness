...
| Source CLI                  | `bin/harness.ts`             | `harness init`, `harness factory status`, `harness factory linear list`, `harness factory linear fetch`, `harness factory triage`, `harness factory planning run`, `harness factory planning publish`, `harness factory planning mark-plan-merged`, `harness run change-review`, `harness run factory-triage`, `harness run plan-review`, `harness runs prune`, `harness models`, `harness skills install` | User-facing harness workflow commands, tracker read adapters, local factory inbox management, and planning handoff metadata commands. Workflow implementations live in `workflows/*.workflow.ts`. |
