...
| `.harness/inbox/factory/*.json` in a workspace   | User-created files, future tracker adapters                                                                                                                    | Workspace-local factory intake queue                                       | Ignored; do not commit                                                   | Pending local factory work items. `harness factory status` reads these files. `harness factory triage --item-file ...` and `harness factory planning --item-file ...` can run one item without moving it.                                                                                                                                                                      |
...
| `.harness/runs/factory/<run-id>/` in a workspace | `harness run factory-triage`, `harness factory triage`, `harness factory planning`, dry-run factory station commands                                           | Workspace-local run state, either external target repo or harness checkout | Ignored; do not commit                                                   | Holds `context/work-item.json` plus station-specific prompts, raw/parsed JSON, summaries, and `meta.json`. Planning runs write `planning/draft.md`, `iterations/<n>/` planner snapshots, and `factoryMetadata` in `meta.json`. Live runs also write `events.jsonl`. Default `harness runs prune` is reviews-only; pass this path with `--runs-dir` or delete manually for now. |
...
and `harness factory planning --item-file ...` create factory run artifacts for
