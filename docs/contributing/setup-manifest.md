...
| `.harness/factory-drafts/<run-id>/draft.md` in a workspace                                 | Planner during factory planning; Harness during dry-run                                                                                                        | Workspace-local planner scratch, separate from durable factory evidence    | Ignored; intentionally retained; never recovery state                    | Planner-owned mutable draft. Harness validates it and publishes canonical and immutable snapshots to the external durable factory store. Remove manually only after containment inspection.                                                                                                                              |
...
| `.harness/inbox/factory/*.json` in a workspace                                             | User-created files, future tracker adapters                                                                                                                    | Workspace-local factory intake queue                                       | Ignored; do not commit                                                   | Pending local factory work items. `harness factory status` reads these files. `harness factory triage --item-file ...` and `harness factory planning run --item-file ...` can run one item without moving it.                                                                                                            |
...
| `${XDG_DATA_HOME:-~/.local/share}/harness/store/projects/<repo-id>/runs/factory/<run-id>/` | `harness factory` triage, planning, and implementation station commands                                                                                        | Durable factory run evidence                                               | User data; do not commit                                                 | Holds station artifacts and `meta.json` with store provenance. Nested factory plan reviews use the same project's `runs/reviews/`. Low-level `harness run factory-triage` remains workspace-local. Prune remains explicit-path based; pass this durable root with `--runs-dir`.                                          |
| `${XDG_DATA_HOME:-~/.local/share}/harness/store/projects/<repo-id>/runs/reviews/<run-id>/` | `harness factory implementation review`                                                                                                                        | Durable implementation-review evidence                                     | User data; do not commit                                                 | Holds review reservations, identity context, nested three-role findings, remediation decisions, candidate/partial-recovery evidence, summaries, checkpoints, and the final PR-ready handoff. Review pointers retain recorded implementation and review roots for custom implementation run directories.                  |
| `dev/plans/*.md` in a workspace                                                            | Approved live `harness factory planning` runs                                                                                                                  | Workspace tracked planning artifacts                                       | Commit intentionally; tracker-backed plans should land through a plan PR | The planning station writes the final reviewed plan under `dev/plans/` after approval. Tracker-backed names should use the issue key, such as `FER-123.md`; local/manual names may use date/title slugs. Dry-run does not write this tracked final plan. Target repos decide whether and when to commit generated plans. |
...
repositories keep workspace-local inbox files and committed material. Harness-owned schemas resolve from the
harness checkout `schemas/` directory. Approved live planning runs can also write tracked
`dev/plans/*.md` in the target workspace. Tracker-backed plans should be
published through a plan PR before the tracker item moves to
`Ready to Implement`. Default `harness runs prune` targets review runs; factory
...
and `harness factory planning run --item-file ...` create factory run artifacts for
one work item and do not move pending inbox files. Current factory station
