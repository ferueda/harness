...
| `.harness/runs/factory/<run-id>/` in a workspace | `harness run factory-triage`, `harness factory triage`, `harness factory planning run`, `harness factory planning publish`, `harness factory planning mark-plan-merged`, dry-run factory station commands | Workspace-local run state, either external target repo or harness checkout | Ignored; do not commit                                                   | Holds `context/work-item.json` plus station-specific prompts, raw/parsed JSON, summaries, and `meta.json`. Planning runs write `planning/draft.md`, `iterations/<n>/` planner snapshots, and `factoryMetadata` in `meta.json`. Publication commands update planning run `meta.json` and `summary.md`. Live runs also write `events.jsonl`. Default `harness runs prune` is reviews-only; pass this path with `--runs-dir` or delete manually for now. |
...
Factory station artifacts follow the same workspace-local rule under
`.harness/runs/factory/<run-id>/`. Harness-owned schemas resolve from the
harness checkout `schemas/` directory; target repos own work-item files and run
artifacts, not schema copies. Approved live planning runs can also write tracked
...
`--runs-dir <workspace>/.harness/runs/factory` or manual deletion until prune
...
Linear read-only fetch and Linear-backed triage input use `LINEAR_API_KEY` for
`harness factory linear fetch` and
`harness factory triage --linear-issue TEAM-123`.
