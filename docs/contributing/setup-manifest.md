...
| `dev/plans/*.md` in a workspace                  | Approved live `harness factory planning` runs                                                                                                                  | Workspace tracked planning artifacts                                       | Commit intentionally when desired          | The planning station writes the final reviewed plan under `dev/plans/` after approval. Default names may include tracker identity, such as `YYMMDD-gh-123-short-slug.md`. Dry-run does not write this tracked final plan. Target repos decide whether and when to commit generated plans.                                                                                      |
...
`dev/plans/*.md` in the target workspace. Default `harness runs prune` targets
review runs; factory run cleanup currently needs
