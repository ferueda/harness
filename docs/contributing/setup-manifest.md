...
| `.harness/inbox/factory/*.json` in a workspace   | User-created files, future tracker adapters, `harness factory dispatch` moves                                                                                  | Workspace-local factory intake queue                                       | Ignored; do not commit                     | Pending local factory work items. `harness factory status` reads these files; live dispatch moves them to `processed/` or `failed/`.                                                                                                                                        |
| `.harness/inbox/factory/processed/*.json`        | `harness factory dispatch`                                                                                                                                     | Workspace-local factory intake history                                     | Ignored; do not commit                     | Successfully dispatched inbox items, renamed with the factory run id prefix.                                                                                                                                                                                                |
| `.harness/inbox/factory/failed/*`                | `harness factory dispatch`                                                                                                                                     | Workspace-local factory intake failure state                               | Ignored; do not commit                     | Failed inbox items and sibling `.error.json` summaries, renamed with the factory run id prefix.                                                                                                                                                                             |
...
| `.harness/runs/factory/<run-id>/` in a workspace | `harness run factory-triage`, dry-run factory triage commands, `harness factory dispatch`                                                                      | Workspace-local run state, either external target repo or harness checkout | Ignored; do not commit                     | Holds `context/work-item.json`, factory prompt, raw/parsed triage JSON, route JSON/markdown, `summary.md`, and `meta.json`. Live runs also write `events.jsonl`. Default `harness runs prune` is reviews-only; pass this path with `--runs-dir` or delete manually for now. |
...
Factory triage artifacts follow the same workspace-local rule under
...
artifacts, not schema copies. Default `harness runs prune` targets review runs;
factory run cleanup currently needs `--runs-dir <workspace>/.harness/runs/factory`
or manual deletion until prune grows a factory-aware default.

Factory inbox files are separate from future review-trigger inbox files.
`harness factory status` is read-only. `harness factory dispatch --dry-run`
creates factory run artifacts but does not move pending inbox files; live
dispatch moves pending files into `processed/` or `failed/`.
