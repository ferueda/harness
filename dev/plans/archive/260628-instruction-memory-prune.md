> **Archive note:** Shipped 2026-06-28. Historical reference only — do not execute step-by-step.
...
- **Shipped**: 2026-06-28 — backup `~/.codex/backups/harness-instruction-prune-20260628-122820.tar.gz`; `AGENTS.md` 111 lines; active roadmap 138 lines; handoff history archived; runs prune no-op (0 runs older than 7d)
...
- [x] A fresh backup exists under `$HOME/.codex/backups/`.
- [x] `AGENTS.md` is under 200 lines.
- [x] `AGENTS.md` no longer duplicates the repo layout, full skill catalog, or
      skill-file templates.
- [x] An active roadmap still exists; if
      `dev/plans/260621-agent-harness-handoff.md` remains active, it is under 250
      lines.
- [x] Ignored `.harness/runs/reviews` handoffs are pruned or a reason is recorded
      for leaving them. (No runs older than 7d; prune dry-run matched 0.)
- [x] `.agents/skills/**` and `skills/**` have no diff.
- [x] `dev/plans/README.md` reflects the active queue after cleanup.
- [x] Final verification commands are run and results recorded.
