...
export function initHarnessConfig(options = {}, cwd = process.cwd()) {
  const workspace = resolveInitWorkspace(options.workspace, cwd);
  ...
  result.gitignoreUpdated = ensureGitignoreEntry(gitignorePath, ".harness/runs/");
  return result;
}
...
function resolveInitWorkspace(explicitWorkspace, cwd) {
  if (explicitWorkspace) {
    return isAbsolute(explicitWorkspace) ? explicitWorkspace : resolve(cwd, explicitWorkspace);
  }
