...
export function createWorkflowContext(options) {
  ...
  const runDir = join(resolve(options.runsDir ?? join(workspace, ".agent-runs/reviews")), runId);
  ...
  return {
    ...
    async agent(name, input = {}) {
      ...
      const prompt = renderPrompt(
        ...
        buildPromptValues(scope, diffSection, options, workspace, input.prior),
      );
...
function buildPromptValues(scope, diffSection, options, workspace, priorReview) {
  return {
    BASE_REF: scope.baseRef,
    HEAD_REF: scope.headRef,
    MERGE_BASE: scope.mergeBase,
    HEAD_SHA: scope.headSha,
    PLAN_SECTION: buildPlanSection(options.planPath, workspace),
    HANDOFF_SECTION: buildHandoffSection(options.handoffPath, workspace),
    DIFF_SECTION: diffSection,
    PRIOR_REVIEW_JSON: priorReview ? JSON.stringify(priorReview, null, 2) : "",
  };
}
