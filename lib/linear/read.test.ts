...
function makeRevisionClient(issues: TestIssue[]) {
  const issueReads = vi.fn<(variables?: unknown) => Promise<TestPage<TestIssue>>>(
    async (variables?: unknown) => {
      const query = variables as QueryVariables;
  ...
  const base = makeFakeClient({}).client;
  return {
    client: { ...base, issues: issueReads } as LinearClientLike,
    issueReads,
  };
}

function revisions(count: number): TestIssue[] {
...
describe("standalone Linear issue context", () => {
  ...
  it("lists only plain issue revisions through exact stable-ID filters", async () => {
    const fake = makeRevisionClient(revisions(51));

    const result = await service(fake.client).listIssueRevisions({
      teamId: TEAM.id,
      projectId: PROJECT.id,
      stateId: READY_STATE.id,
      limit: 51,
  ...
  it("accepts the exact pilot limit and reports the first excess issue", async () => {
    const exact = makeRevisionClient(revisions(250));
    const overflow = makeRevisionClient(revisions(251));

    await expect(
      service(exact.client).listIssueRevisions({
        teamId: TEAM.id,
        projectId: PROJECT.id,
        stateId: READY_STATE.id,
        limit: 250,
    ...
    await expect(
      service(overflow.client).listIssueRevisions({
        teamId: TEAM.id,
        projectId: PROJECT.id,
        stateId: READY_STATE.id,
        limit: 250,
  ...
  it("validates revision-list inputs and normalizes upstream failures", async () => {
    const fake = makeRevisionClient(revisions(1));
    await expect(
      service(fake.client).listIssueRevisions({
        teamId: " ",
        projectId: PROJECT.id,
        stateId: READY_STATE.id,
        limit: 250,
    ...
    const failed = makeFakeClient({ issueFailure: cause });
    const error = await service(failed.client)
      .listIssueRevisions({
        teamId: TEAM.id,
        projectId: PROJECT.id,
        stateId: READY_STATE.id,
