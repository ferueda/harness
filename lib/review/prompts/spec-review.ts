...
- Challenge needlessly horizontal phases such as completing all persistence, then all services, then all interfaces when useful behavior could land end to end. Keep shared setup to the minimum required by the first slice and expand it only when a later slice proves the need.
- Accept a horizontal step when the plan gives a short, evidence-backed reason vertical delivery is impractical or unsafe, such as an indivisible migration, a cross-cutting safety fix, or the smallest shared prerequisite.
- Treat delivery shape as judgment, not a ban. Request restructuring only when the horizontal breakdown needlessly delays observable proof, prevents focused review, or creates avoidable dependency or collision risk.
...
- **Testing**: prefer the highest existing stable seam proving acceptance; require the cheapest credible acceptance-level proof and add another seam only for a distinct boundary or failure mode.

### Outcome proof

- Connect each material outcome or forbidden effect to an exact proof action and expected observable evidence. The canonical repository gate proves general health; it does not replace behavioral proof.
- Require material proof limits to be explicit. A mock, fake, intercepted request, or source-only check does not prove behavior beyond its seam.
- For asynchronous work, require the terminal state or downstream effect. Acceptance or enqueueing alone is insufficient.
...
- Accept focused proof plus the repository gate when it proves the outcome. Do not demand repeated layers without a distinct failure mode.
- Require unavailable material proof to be reported with its concrete blocker, and require handoff evidence to name observed results and skipped checks.
