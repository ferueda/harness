...
import { canonicalSpecReviewFinding, createSpecReviewFindingId } from "./finding-identity.ts";
import {
...
function addTrustedFindingIds(
  ...
  for (const draft of drafts) {
    const canonical = canonicalSpecReviewFinding(draft);
    ...
    const id = createSpecReviewFindingId({
      artifact,
      rubricVersion: SPEC_REVIEW_RUBRIC_VERSION,
      finding: draft,
    });
